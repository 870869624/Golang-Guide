...
```bash
# 查看镜像
docker images --no-trunc | grep ngdlp

# 在构建机器上
docker save -o ngdlp-new.tar docker-hub.cloud.top/ngdlp/ngdlp:latest
...
# 拷贝到目标机器并加载
docker load -i ngdlp_1.0.30-hotfix-20260317.tar

k3s ctr images import ngdlp_1.0.30-hotfix-20260317.tar
##或者
ctr -n k8s.io images import ngdlp-new.tar

```

**方法1.1：保存为tar文件**
```bash
1. 上传补丁镜像文件ngdlp_1.0.30-hotfix-20260317.tar到任一目录；
2. 加载镜像：/home/topsec/cache/gcrpi -f ngdlp_1.0.30-hotfix-20260317.tar
3. 修改镜像tag：kubectl edit deploy -n topsec-topngdlp ngdlp
4. 将topsec-epsec.com.cn:33500/ngdlp/ngdlp:1.0.30 修改为 topsec-epsec.com.cn:33500/ngdlp/ngdlp:1.0.30-hotfix-20260317， 然后保存；等待ngdlp POD重启完成即可

##再次上传同标签镜像只需要重启pod就可以
```
