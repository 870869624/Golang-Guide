...
k3s ctr images import clean_engine.tar(导入镜像)

kubectl -n sdsp rollout undo deployments/cleansvc(回滚上个版本)
```
...
### 查看日志：
```bash
kubectl logs -f cleansvc-686cdf4d4f-g5vgt -n sdsp
```
...
kubectl exec -it cleansvc-6465767ff8-kh9j9 -n sdsp -c cleansvc -- sh
```
```bash
...
## 新增pod的netport
```bash
kubectl expose pod node-5cf8d66889-b452n -n sdsp --name=node-rpc-8282 \
...
# 下载文件
```bash
scp -P 1022 -o ServerAliveInterval=50 -l 10240 root@10.34.11.45:/mnt/hgfs/shareLinux/ag-vsp/agcbb.tar.gz C:\Users\linrui\Desktop\fsdownload
```